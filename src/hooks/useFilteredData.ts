...
export const useFilteredData = () => {
  ...
  const clientesData = useMemo(() => {
    return filteredClientes.slice(0, 8).map((c) => {
      const reservas =
        filteredReservas.filter((r) => r.cliente === c.nome).reduce((a, r) => a + r.quantidade, 0) ||
        Math.floor(500 + Math.random() * 1500);
      const vendas =
        filteredVendas.filter((v) => v.cliente === c.nome).reduce((a, v) => a + v.quantidade, 0) ||
        Math.floor(reservas * (0.7 + Math.random() * 0.3));
      return {
        nome: c.nome,
