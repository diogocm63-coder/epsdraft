...
import { gerarOrcamentoHoras, areaHa, gerarHorasTrabalhadas, gerarCustos } from '@/data/operacoesData';
...
const OrcamentacaoProdutosPage = () => {
  ...
  // Modified header data for this page
  const horasData = { total: 605, horasUteis: 511, horasPausa: 95, percentUteis: 84, percentPausa: 16 };
  const custosData = { total: 0, pessoal: 0, produtos: 0, percentPessoal: 0, percentProdutos: 0 };
    ...
    >
      <div className="space-y-4 h-full overflow-auto">
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                {custoOrcamentado.map((val, i) => (
                  <TableCell key={i} className="text-xs text-center">{val > 0 ? `${val.toFixed(2).replace('.', ',')} €` : ''}</TableCell>
                ))}
                <TableCell className="text-xs text-center font-bold">{totalOrcamentado.toFixed(2).replace('.', ',')} €</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </div>
        ...
        <div className="bg-white rounded-lg border p-4">
          <h3 className="text-[#8B1538] font-semibold text-sm mb-2">Orçamento de Quantidades</h3>
        ...
        {/* Orçamento de €/Qtd */}
        <div className="bg-white rounded-lg border p-4">
          <h3 className="text-[#8B1538] font-semibold text-sm mb-2">Orçamento de €/Qtd</h3>
          <div className="flex gap-2 mb-3">
            <Button variant="outline" size="sm" className="text-xs h-7">
              <Save className="h-3 w-3 mr-1" /> Save Changes
            </Button>
