...
import { LayoutDashboard, Users, Store, ChevronLeft, ClipboardList, DollarSign, Users2, ListTodo, BarChart3, PieChart, Clock, Package } from 'lucide-react';
...
export const DashboardSidebar = () => {
        ...
        {!collapsed && (
          <div className="flex flex-col">
            ...
            {showSubtitle && item.subtitle && (
              <span className={`text-[10px] ${isActive ? 'text-sidebar-primary-foreground/70' : 'text-sidebar-foreground/60'}`}>
                {item.subtitle}
              </span>
            )}
          </div>
  ...
  return (
    <aside className={`${collapsed ? 'w-14' : 'w-52'} min-h-screen bg-sidebar flex flex-col transition-all duration-300`}>
      {/* Navigation */}
