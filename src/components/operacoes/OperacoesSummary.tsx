...
export const OperacoesSummary = ({
  showDashboardLabel = true
}: OperacoesSummaryProps) => {
      ...
      <div className="grid grid-cols-4 gap-3">
          ...
          <div className="flex items-center justify-center">
            <div className="relative w-20 h-20">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={tipoAtividadePie} cx="50%" cy="50%" innerRadius={22} outerRadius={35} dataKey="value" startAngle={90} endAngle={-270}>
                    {tipoAtividadePie.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
        ...
        <div className="bg-white border border-gray-200 p-3 rounded-lg">
          ...
          <div className="h-14 mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={tipoAtividadeBar} layout="vertical" margin={{
              top: 0,
              right: 10,
              left: 0,
              bottom: 0
      ...
      {showDashboardLabel && <div className="flex items-center justify-between border-b border-gray-200 pb-1">
          ...
          <div className="flex items-center gap-3">
            <div className="border border-gray-300 rounded px-3 py-1 text-xs bg-white">
              ...
              <span className="font-bold ml-2">{summaryData.areaHa}</span>
            </div>
            <div className="border border-gray-300 rounded px-3 py-1 text-xs bg-white">
              <span className="text-gray-600">% Execução:</span>
              <span className="font-bold ml-2">{summaryData.execucao}%</span>
            </div>
          </div>
        </div>}
    </div>;
